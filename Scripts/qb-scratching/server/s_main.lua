...
QBCore.Functions.CreateUseableItem('scratch_ticket', function(source)
  ...
  TriggerClientEvent("qb-scratching:isActiveCooldown", source)
end)

RegisterNetEvent("qb-scratching:handler", function(returncooldown, cooldown)
  ...
  if count >= 1 then
    ...
    TriggerClientEvent("qb-scratching:setCooldown", _source)
  ...
  TriggerClientEvent("qb-scratching:startScratchingEmote", _source)
  ...
  for key,priceInfo in pairs(Config.Prices) do
    ...
    if randomNumber > add and randomNumber <= add + chance then
      ...
      TriggerClientEvent("qb-scratching:nuiOpenCard", _source, key, price, amount, price_type, price_label)
      return price
    end
    add = add + chance
  end
...
RegisterNetEvent("qb-scratching:deposit", function(key, price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
      ...
      elseif type == 'money' and giveMoney then
        if tonumber(amount) == priceAmount then
          if tonumber(price) > 0 then
            ...
            xPlayer.Functions.AddMoney("cash", tonumber(price), 'Scratch Card')
          else
...
RegisterNetEvent("qb-scratching:stopScratching", function(price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
  local tempsrc = tonumber(_source)
