...
function OpenCrate(netId)
    if lib.progressBar({
        ...
        TriggerServerEvent("gundrop:server:openCrate", netId)
    end
end
...
function CrateDrop(planeSpawnDistance, dropCoords)
    CreateThread(function()
        ...
        SetTimeout(Config.TimeUntilDrop * 60 * 1000, function()
            ...
            TriggerServerEvent("gundrop:server:showTarget", NetworkGetNetworkIdFromEntity(crate))
        end)
    end)
end

RegisterNetEvent("gundrop:client:showTarget", function(netId)
    if Config.Target == 'ox_target' then
        exports.ox_target:addEntity(netId, {
            {
                name = 'gundrop:drops:opencrate',
...
RegisterNetEvent("gundrop:client:removeTarget", function(netId)
    if Config.Target == 'ox_target' then
        exports.ox_target:removeEntity(netId, 'gundrop:drops:opencrate')
    else
        local _crate = NetworkGetEntityFromNetworkId(netId)
...
RegisterNetEvent("gundrop:client:startDrop", function(planeSpawnDistance, dropCoords)
    CreateThread(function()
...
RegisterNetEvent("gundrop:client:createDrop", function()
    ...
    PoliceAlert()
    TriggerEvent("gundrop:client:startDrop", Config.PlaneSpawnDistance, vector3(playerCoords.x, playerCoords.y, playerCoords.z))
end)
