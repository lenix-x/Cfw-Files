                ...
                if nearDefault then
                    ...
                    if nearDefault then
                        if not isPlyInBennys then
                            Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "[Tryk ~p~E~w~ - Gå Ind I Benny's Motorworks]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
...
RegisterNetEvent("qb-customs:purchaseSuccessful", function()
    ...
    QBCore.Functions.Notify("Køb Succesfuldt")
end)

RegisterNetEvent("qb-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Ikke nok penge", "error")
end)
