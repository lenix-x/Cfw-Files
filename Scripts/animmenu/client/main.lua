        ...
        elseif data.category == "syncedemotes" then
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                ...
                for _, id in pairs(nearbyPlayers) do
                    exports['0r-textui']:create3DTextUIOnPlayers("-request-players-" .. id, {
                        id = id,
                        displayDist = 5.0,
                        interactDist = 1.3,
                        ...
                        theme = "green", -- or red
                        triggerData = {
                            triggerName = ":sendAnimRequest:client",
                ...
                Citizen.CreateThread(function()
                    while true do
                        ...
                        if IsControlPressed(0, 322) then
                            ...
                            for _, id in pairs(nearbyPlayers) do
                                exports['0r-textui']:delete3DTextUIOnPlayers("-request-players-" .. id)
                            end
                            break
                        end
                    end
...
RegisterNetEvent('animmenu:receiveAnimRequest:client', function(data)
    ...
    exports['0r-textui']:create3DTextUIOnPlayers("-request-player-" .. data.target, {
        id = data.target,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true, -- If true when you near it and click key it will trigger the event that you write inside triggerData
        ...
        theme = "green", -- or red
        triggerData = {
            triggerName = ":playAnimTogetherReceiver:client",
            args = {data = data}
        }
    })
    Citizen.CreateThread(function()
        while true do
            ...
            if IsControlPressed(0, 322) then
                ...
                exports["0r-textui"]:hideTextUI()
                exports['0r-textui']:delete3DTextUIOnPlayers("-request-player-" .. data.target)
...
RegisterNetEvent('animmenu:playAnimTogetherReceiver:client', function(data)
    requestReceiveActive = false
    exports["0r-textui"]:hideTextUI()
    exports['0r-textui']:delete3DTextUIOnPlayers("-request-player-" .. data.data.target)
    local inVehicle = IsPedInAnyVehicle(PlayerPedId(), true)
    if inVehicle then return end
