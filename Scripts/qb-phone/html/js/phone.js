...
$(document).on('click', '#incoming-cancel2', function(e){
    e.preventDefault();
    $.post('https://qb-phone/CancelOutgoingCall');
});
...
$(document).on('click', '#incoming-deny2', function(e){
    e.preventDefault();
    $.post('https://qb-phone/DenyIncomingCall');
});
...
// QB.Phone.Animations.BottomSlideUp('.container', 450, -49);
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            $("#incoming-answer2").css({"display":"flex"});
            $("#incoming-deny2").css({"display":"flex"});
            $("#incoming-cancel2").css({"display":"none"});
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        // $(".phone-call-ongoing-caller").html(cData.TargetData.name);
        // $(".phone-call-outgoing-caller").html(cData.TargetData.name);
...
$(document).on('click', '.phone-currentcall-nowing', function(e){
    e.preventDefault();
    ...
    $(".phone-call-outgoing-caller").html(CallData.name);
    $(".phone-call-incoming-caller").html(CallData.name);
...
$(document).on('click', '#incoming-answer2', function(e){
    e.preventDefault();
    $.post('https://qb-phone/AnswerCall');
});
...
QB.Phone.Functions.AnswerCall = function(CallData) {
    ...
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
    $("#incoming-cancel2").css({"display":"flex"});
    $("#incoming-answer2").css({"display":"none"});
    $("#incoming-deny2").css({"display":"none"});
    $(".phone-call-ongoing-caller").html(CallData.TargetData.name);
    $(".phone-call-incoming-caller").html(CallData.TargetData.name);
    $(".phone-call-outgoing-caller").html(CallData.TargetData.name);
    if (QB.Phone.Data.currentApplication == "phone-call") {
        QB.Phone.Animations.TopSlideUp('.phone-application-container', 400, -200);
        QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -200);
        setTimeout(function(){
            QB.Phone.Functions.ToggleApp(QB.Phone.Data.currentApplication, "none");
        }, 400)
        QB.Phone.Functions.HeaderTextColor("white", 300);
